...
- **CMS**: Payload CMS 3.x (shares Supabase PostgreSQL, separate `payload` schema)
...
Payload CMS is used for content management (blog posts, etc.) and shares the same PostgreSQL database as Supabase but uses a separate schema.

**Configuration**: `payload.config.ts`

**Key Setup Details**:
- **Database**: Uses `@payloadcms/db-postgres` adapter with `schemaName: 'payload'` to isolate from Supabase tables
- **Admin UI**: Available at `/admin` (separate from main app auth)
- **Email**: Uses `@payloadcms/email-resend` adapter (shares Resend API key)
- **Rich Text**: Lexical editor (`@payloadcms/richtext-lexical`)

**Collections**: Defined in `src/collections/`
- `Users.ts` - Payload admin users (separate from Supabase auth users)

**Important Files**:
- `payload.config.ts` - Main configuration
- `app/(payload)/layout.tsx` - Payload admin layout with serverFunction
- `app/(payload)/admin/importMap.js` - Auto-generated by Payload (empty by default)
- `payload-types.ts` - Auto-generated TypeScript types

**Adding a New Collection**:
1. Create collection file in `src/collections/`
2. Import and add to `collections` array in `payload.config.ts`
3. Run dev server to auto-migrate schema
4. Payload generates types in `payload-types.ts`
...
│   ├── api/            # Payload REST API at /api (Payload-specific)
...
└── collections/        # Payload CMS collection definitions
    └── Users.ts        # Payload admin users collection
...
The app uses **two separate route groups** with independent root layouts to prevent nested HTML document errors:
...
- **`app/(payload)/`** - Payload CMS admin
  - Has its own `<html>`, `<head>`, `<body>` via Payload's `RootLayout`
  - Completely isolated styling and scripts
  - Admin UI at `/admin`

- **`app/layout.tsx`** - Minimal root layout
  - Simply returns `{children}` without any HTML wrapper
  - Allows each route group to define its own document structure
